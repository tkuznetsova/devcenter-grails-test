			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="addressline1" title="${message(code: 'user.addressline1.label', default: 'Addressline')}"/>
				...
				<g:each in="${userInstanceList}" status="i" var="userInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
							...
							<td><g:link action="show" id="${userInstance.id}">${userInstance.name}</g:link></td>
						
							<td>${fieldValue(bean: userInstance, field: "creditcardID")}</td>
						
							<td>${fieldValue(bean: userInstance, field: "phone")}</td>
						
							<td>${fieldValue(bean: userInstance, field: "addressline1")} ${fieldValue(bean: userInstance, field: "addressline2")}</td>
